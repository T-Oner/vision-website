...
由于周期的评分满分为 100 分，而总权重也是 100，那么在周期评审中，关键结果能给周期贡献的最大分值等于它的权重值。

## 关键结果的属性

关键结果应包含一个**初始值**、一个**目标值**，结合周期进行中*记录*的一个或多个**当前值**。有以下几种规则判断它是否达成：
...
在 Vision，关键结果需要你在周期进行中，定时或不定时记录你的当前值。我们会通过图表的形式展示出该指标的变化的趋势。
...
<!-- 可以把评审想象为考试，而周期的执行过程理解为考试前的学习。虽然很残酷，但评审最终看重的是你考试的最终得分，无论你学习的过程有多么的努力。 -->

### 评分

评分是指，在周期评审过程中，根据每个关键结果对目标的**贡献**，由用户给出一个结合客观进度和主观判断的分数。

:::info 提示
这里的贡献是指，关键结果对目标的实现产生的积极的作用。
:::
